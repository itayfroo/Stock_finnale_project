...
class Users:
    ...
    def __init__(self):
        company_dict = self.load_company_dict()


    def load_company_dict(self):
        try:
            with open(r"texts\stocks.json", "r") as json_file:
                self.company_dict = json.load(json_file)
        except FileNotFoundError:
            return {}
    ...
    def sign_up(self,username, password, additional_info="default_value"):
        if os.path.exists(json_file_path):
            with open(json_file_path, "r") as file:
                file_contents = file.read()
                if file_contents:
        ...
        if self.user_exists(username):
            with open(json_file_path, "r") as file:
                ...
                if user_data and user_data.get("password") == password:
                    ...
                    st.button(translate_word('Okay'), on_click=self.click_button)
                    ...
                    stocks = [""] + list(self.company_dict.keys())
                    ...
                    if (additional_info['Amount_invested']==0 or additional_info['Amount_invested']==""):
                        ...
                        st.button('Confirm', on_click=self.click_button)
                        if st.session_state.clicked:
                            try:
