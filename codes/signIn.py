...
api_keys = ['MNI5T6CU7KLSFJA8', 'QJFF49AEUN6NX884', '9ZZWS60Q2CZ6JYUK', 'ZX5XTAKCAXGAYNBG',
            "XUKT2LY2NIC35B83","9XZBYP0RSJFMOT4L","L485NGI7NK2M6VFT","PS74H4D0OXVW2M22",
            "X7RFFB0EHKNTH25O","EEINBBF6PX2GAO02","FLTAY1Z6W73ZVRQB","JDZLDTK95XWAYVEP",
...
def sign_up(username, password, additional_info="default_value"):
    ...
    if username in users:
        ...
        users[f"{username}_info"] = {'Age': age, 'City': city,
...
def get_stock_data(symbol, start_date, end_date):
    try:
        ...
        if stock_data.empty:
            st.caption(translate_word("Check again tomorrow(:"))
            return None
...
def end(username, password):
    ...
    # Stock calculations
    try:
        amount_invested = float(additional_info.get('Amount_invested', 0))
        if amount_invested > 0:
            stock_name = additional_info.get('Stock_investment', "")
            if stock_name:
                stock_symbol = get_stock_symbol(stock_name)
                start_date = datetime.strptime(additional_info.get('date'), "%Y-%m-%d %H:%M:%S.%f")
                ...
                if stock_data is not None:
                    ...
                    gain_loss = amount_invested * (1 + percent_change / 100)

                    st.text(translate_word(f"Based on current stock prices, you would now have: {gain_loss:.2f}$"))
                    st.text(translate_word(f"with the percentage of change: {percent_change:.2f}%"))

                    if gain_loss > amount_invested:
                        st.text(translate_word(f"Your total gain: {(gain_loss - amount_invested):.2f}$"))
